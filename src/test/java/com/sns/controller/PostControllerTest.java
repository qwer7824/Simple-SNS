...
import com.sns.controller.request.PostCommentRequest;
...
import com.sns.fixture.PostEntityFixture;
...
public class PostControllerTest {
    ...
    void 포스트작성() throws Exception{
        ...
        mockMvc.perform(post("/api/v1/posts")
                        ...
                        .content(objectMapper.writeValueAsBytes(new PostCrateRequest(title,body)))
                ).andDo(print())
    ...
    @Test
    @WithMockUser
    void 포스트삭제시_삭제하려는_포스트가_존재하지않을_경우() throws Exception{
    ...
    @Test
    @WithAnonymousUser
    void 피드목록요청시_로그인하지_않는경우() throws Exception{
    ...
    @Test
    @WithAnonymousUser
    void 내피드목록요청시_로그인하지_않는경우() throws Exception{
    ...
    @Test
    @WithAnonymousUser
    void 좋아요버튼클릭시_로그인하지_않는경우() throws Exception{
        mockMvc.perform(post("/api/v1/posts/1/likes")
                        .contentType(MediaType.APPLICATION_JSON)
                ).andDo(print())
                .andExpect(status().isUnauthorized());
    ...
    @Test
    @WithAnonymousUser
    void 댓글작성시_로그인하지_않는경우() throws Exception{
        mockMvc.perform(post("/api/v1/posts/1/comments")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsBytes(new PostCommentRequest("comment")))
                ).andDo(print())
